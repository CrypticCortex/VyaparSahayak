...
- [x] 1. Orders API Routes
  - [x] 1.1 Create GET /api/orders route in `src/app/api/orders/route.ts`
  ...
  - [x] 1.2 Create PATCH /api/orders/[id] route in `src/app/api/orders/[id]/route.ts`
  ...
  - [x] 1.3 Create POST /api/orders/batch route in `src/app/api/orders/batch/route.ts`
  ...
  - [x] 1.4 Create GET /api/orders/batches route in `src/app/api/orders/batches/route.ts`
  ...
  - [x] 1.5 Create PATCH /api/orders/batch/[id] route in `src/app/api/orders/batch/[id]/route.ts`
  ...
  - [x] 1.6 Add cache helpers in `src/lib/cache.ts`
  ...
  - [x] 1.7 Write tests for orders API routes
...
- [x] 2. Orders Dashboard Page and Components
  - [x] 2.1 Create OrderDashboard server component at `src/app/demo/orders/page.tsx`
  ...
  - [x] 2.2 Create ZoneOrderGroup client component at `src/components/dashboard/zone-order-group.tsx`
  ...
  - [x] 2.3 Create OrderCard client component at `src/components/dashboard/order-card.tsx`
  ...
  - [x] 2.4 Create SuggestionCard client component at `src/components/dashboard/suggestion-card.tsx`
  ...
  - [x] 2.5 Write tests for orders dashboard components
...
- [x] 3. Dispatch Batches Page and Components
  - [x] 3.1 Create BatchList server component at `src/app/demo/orders/batches/page.tsx`
    ...
    - _Refs: R-batch-list, R-batch-grouped-by-status_
  - [x] 3.2 Create BatchCard client component at `src/components/dashboard/batch-card.tsx`
  ...
  - [x] 3.3 Write tests for batch components
...
- [x] 4. Navigation and Dashboard Integration
  - [x] 4.1 Add Orders tab to BottomNav in `src/components/dashboard/bottom-nav.tsx`
  ...
  - [x] 4.2 Update demo layout in `src/app/demo/layout.tsx`
  ...
  - [x] 4.3 Add orders hero card and quick action to main dashboard at `src/app/demo/page.tsx`
...
- [x] GET /api/orders returns orders grouped by zone with correct totals
- [x] PATCH /api/orders/[id] updates order status with appropriate timestamps
- [x] POST /api/orders/batch creates batch, join records, and cascades status to orders
- [x] GET /api/orders/batches returns batches grouped by status with nested orders
- [x] PATCH /api/orders/batch/[id] cascades status change to all orders in the batch
- [x] /demo/orders page renders summary bar, suggestion cards, and zone-grouped orders
- [x] Confirm/Reject buttons work with optimistic UI updates on pending orders
- [x] "Create Dispatch Batch" creates a batch and updates order statuses in the UI
- [x] /demo/orders/batches page renders batches grouped by planned/dispatched/delivered
- [x] BatchCard expands to show individual orders and supports status transitions
- [x] BottomNav has an Orders tab with pending count badge
- [x] Main dashboard (/demo) has Pending Orders hero card and View Orders quick action
- [x] Invalid status transitions return 400 errors
- [x] Empty states render helpful messages instead of blank space
- [x] All test files pass: API route tests, component tests
- [x] Pages are usable on 375px mobile viewport
