...
- [x] 1. Install Dependencies and Set Up Module Structure
  - [x] 1.1 Install @strands-agents/sdk and zod packages
  ...
  - [x] 1.2 Create the `src/lib/strands/` directory structure
...
- [x] 2. Configure the Bedrock Model
  - [x] 2.1 Implement `src/lib/strands/model.ts`
...
- [x] 3. Rewrite Existing 11 Tools with Zod Schemas
  - [x] 3.1 Rewrite `get_dashboard_summary` tool
  ...
  - [x] 3.2 Rewrite `scan_inventory` tool
  ...
  - [x] 3.4 Rewrite `get_alert_detail` tool
    ...
    - _Refs: R-tool-rewrite, R-backward-compat_
  - [x] 3.5 Rewrite `generate_recommendation` tool
  ...
  - [x] 3.7 Rewrite `get_campaign_detail` tool
    ...
    - _Refs: R-tool-rewrite, R-backward-compat_
  - [x] 3.8 Rewrite `send_campaign` tool (needs-approval)
  ...
  - [x] 3.10 Rewrite `auto_handle_dead_stock` tool
    ...
    - _Refs: R-tool-rewrite, R-backward-compat_
  - [x] 3.11 Rewrite `get_network_overview` tool
    - Move callback logic from `getNetworkOverview()` in `src/lib/agent-tools.ts` (lines 535-561)
    - _Refs: R-tool-rewrite, R-backward-compat_

- [x] 4. Implement 7 New Tools
  - [x] 4.1 Implement `get_pending_orders` tool
  ...
  - [x] 4.2 Implement `confirm_order` tool (needs-approval)
  ...
  - [x] 4.3 Implement `reject_order` tool (needs-approval)
    ...
    - _Refs: R-new-tools, R-two-tier-autonomy_
  - [x] 4.4 Implement `suggest_dispatch_batch` tool
  ...
  - [x] 4.5 Implement `create_dispatch_batch` tool (needs-approval)
  ...
  - [x] 4.6 Implement `check_retailer_activity` tool
  ...
  - [x] 4.7 Implement `get_campaign_performance` tool
  ...
  - [x] 4.8 Implement `send_campaign_reminder` tool
...
- [x] 5. Implement Autonomy Classification
  - [x] 5.1 Create `src/lib/strands/autonomy.ts`
...
- [x] 6. Update the Chat API Route
  - [x] 6.1 Refactor `src/app/api/chat/route.ts` for Strands Agent
    ...
    - _Refs: R-sdk-migration_
  - [x] 6.2 Implement the new Bedrock mode handler
  ...
  - [x] 6.3 Update the system prompt for 18 tools
  ...
  - [x] 6.4 Preserve demo mode and campaign card collection
...
- [x] 7. Testing
  - [x] 7.1 Write unit tests for Zod tool schemas
  ...
  - [x] 7.2 Write unit tests for tool callbacks
  ...
  - [x] 7.3 Write integration tests for the updated chat route
...
- [x] 8. Cleanup
  - [x] 8.1 Mark `src/lib/agent-tools.ts` as deprecated
    ...
    - _Refs: R-sdk-migration_
  - [x] 8.2 Verify the build compiles cleanly
...
- [x] @strands-agents/sdk and zod are in package.json and the project builds without errors
- [x] All 18 tools are defined with Zod schemas and typed callbacks in `src/lib/strands/tools.ts`
- [x] The 11 existing tools produce identical output shapes to the current implementation
- [x] The 7 new tools return correct data structures (or suggestion cards for mutating tools)
- [x] `src/app/api/chat/route.ts` uses `Agent.invoke()` instead of a manual while loop
- [x] BedrockModel is configured from environment variables with sensible defaults
- [x] Two-tier autonomy: read tools auto-execute, mutating tools return suggestion cards
- [x] Demo mode is unchanged and works without Bedrock credentials
- [x] API response shape is backward compatible: `{ reply, campaigns?, suggestions?, openWhatsApp? }`
- [x] Campaign card collection from tool results still works
- [x] System prompt is updated to describe all 18 tools and autonomy rules
- [x] Unit tests pass for Zod schemas, tool callbacks, and route handler
- [x] No TypeScript compilation errors
- [x] `src/lib/agent-tools.ts` is marked deprecated and no longer imported by the chat route
