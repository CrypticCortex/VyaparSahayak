...
- [x] 1. Token Generation and Campaign Integration
  - [x] 1.1 Create token generator utility in `src/lib/order-token.ts`
  ...
  - [x] 1.2 Update `src/app/api/recommend/[id]/route.ts` to generate tokens on campaign creation
  ...
  - [x] 1.3 Append order link to WhatsApp message text
  ...
  - [x] 1.4 Update mock WhatsApp message in `src/lib/bedrock.ts`
...
- [x] 2. Order Info API Route
  - [x] 2.1 Create `src/app/api/order/[token]/info/route.ts`
  ...
  - [x] 2.2 Handle edge cases in the info route
...
- [x] 3. Order Creation API Route
  - [x] 3.1 Create `src/app/api/order/route.ts`
  ...
  - [x] 3.2 Create Order and OrderItem records
...
- [x] 4. Order Page Layout and UI
  - [x] 4.1 Create `src/app/order/layout.tsx`
  ...
  - [x] 4.2 Create `src/app/order/[token]/page.tsx`
...
- [x] 5. Confirmation Page
  - [x] 5.1 Create `src/app/order/[token]/confirmation/page.tsx`
...
- [x] 6. Testing and Verification
  - [x] 6.1 Write unit tests for token generator
    ...
    - _Refs: R-token-generation_
  - [x] 6.2 Write tests for order info API
    ...
    - _Refs: R-order-info-api_
  - [x] 6.3 Write tests for order creation API
...
- [x] Campaign creation via /api/recommend/[id] generates a `vyp_` token and saves it to campaign.orderLink
- [x] WhatsApp message text includes an order link with the token
- [x] Mock WhatsApp message in bedrock.ts includes an order link placeholder
- [x] GET /api/order/[token]/info returns campaign + product data for valid tokens and 404 for invalid ones
- [x] POST /api/order creates Order + OrderItem records and returns an order number
- [x] /order/[token] renders a mobile-first public order page with poster, price, quantity stepper, and submit button
- [x] /order/[token]/confirmation renders the order number and confirmation message
- [x] Order pages use a minimal layout (logo only, no demo dashboard chrome)
- [x] Invalid tokens show a user-friendly error page, not a crash
- [x] Submit button is disabled during submission to prevent double-tap
- [x] All unit tests pass for token generator, order info API, and order creation API
- [x] Manual smoke test confirms the full flow from campaign creation through order placement to confirmation
