...
- [x] 1. Prisma Schema -- New Models
  - [x] 1.1 Add Order model to `prisma/schema.prisma`
  ...
  - [x] 1.2 Add OrderItem model to `prisma/schema.prisma`
  ...
  - [x] 1.3 Add DispatchBatch model to `prisma/schema.prisma`
  ...
  - [x] 1.4 Add DispatchBatchOrder join model to `prisma/schema.prisma`
  ...
  - [x] 1.5 Add AgentSuggestion model to `prisma/schema.prisma`
  ...
  - [x] 1.6 Update existing models with new reverse relations
  ...
  - [x] 1.7 Run `npx prisma validate` and `npx prisma db push` to verify schema
...
- [x] 2. Seed Script -- Updated Deletion Order
  - [x] 2.1 Update the deletion block in `src/app/api/seed/route.ts`
  ...
  - [x] 2.2 Verify re-running seed on a populated database succeeds without FK errors
    - Run POST /api/seed twice in sequence
    - Confirm second run completes without constraint violations
    - _Refs: R-seed-idempotent_

- [x] 3. Seed Script -- Order Token and Campaign Link Generation
  - [x] 3.1 Add a `generateOrderToken(index: number)` helper function
  ...
  - [x] 3.2 After campaign creation in the seed route, update each campaign with an orderLink
...
- [x] 4. Seed Script -- Demo Orders
  - [x] 4.1 Define demo order configurations
  ...
  - [x] 4.2 Implement order creation loop in the seed route
  ...
  - [x] 4.3 Optionally link some orders to campaigns
...
- [x] 5. Seed Script -- Dispatch Batch
  - [x] 5.1 Create 1 DispatchBatch for TN-URB zone
  ...
  - [x] 5.2 Create DispatchBatchOrder records linking the 2 dispatched orders
...
- [x] 6. Seed Script -- Agent Suggestions
  - [x] 6.1 Define 4 agent suggestion records with varied types and priorities
  ...
  - [x] 6.2 Insert suggestions using `prisma.agentSuggestion.createMany()` or individual creates
...
- [x] 7. Seed Script -- Updated Stats Response
  - [x] 7.1 Update the stats object returned by the seed endpoint
...
- [x] `npx prisma validate` passes with zero errors on the updated schema
- [x] `npx prisma db push` creates all five new tables and the new Campaign.orderLink column in SQLite
- [x] Prisma client is regenerated and TypeScript compiles without type errors
- [x] POST /api/seed succeeds on a fresh database and returns updated stats
- [x] POST /api/seed succeeds on a previously seeded database (re-run without FK errors)
- [x] Database contains 11 orders: 6 pending, 3 confirmed, 2 dispatched
- [x] Each order has 1-4 OrderItems with valid product and pricing references
- [x] 1 DispatchBatch exists for TN-URB with status "dispatched" and 2 linked orders
- [x] 3-4 AgentSuggestion records exist with varied types, priorities, and valid JSON payloads
- [x] All Campaign records have a non-null orderLink value after seeding
- [x] No existing seed functionality is broken -- distributor, zones, retailers, products, sales, inventory, alerts, recommendations, campaigns, and WhatsApp groups still seed correctly
