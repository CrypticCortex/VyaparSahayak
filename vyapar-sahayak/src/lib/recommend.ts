...
// Shared recommendation logic used by both the API route and agent tools.
...
export interface RecommendResult {
  success: true;
  recommendation: { id: string; type: string; [key: string]: unknown };
  campaignId: string;
...
export async function generateRecommendationForAlert(
  alertId: string
): Promise<RecommendResult | RecommendError> {
  const alert = await prisma.deadStockAlert.findUnique({ where: { id: alertId } });
  if (!alert) {
