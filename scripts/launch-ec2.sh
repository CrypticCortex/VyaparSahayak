...
eval "$AWS amplify update-branch \
  ...
  'BEDROCK_REGION=us-east-1,BEDROCK_TEXT_MODEL=amazon.nova-lite-v1:0,CHAT_USE_BEDROCK=true,DATABASE_URL=REDACTED_TURSO_URL,TURSO_AUTH_TOKEN=${TURSO_AUTH_TOKEN},BEDROCK_ACCESS_KEY_ID=${BEDROCK_ACCESS_KEY_ID},BEDROCK_SECRET_ACCESS_KEY=${BEDROCK_SECRET_ACCESS_KEY},EC2_REDIRECT_URL=http://${ELASTIC_IP}'" \
  &>/dev/null
echo "  Done."
